...
dat_samplef <- dat %>%
  filter(family_id %in% sample_fams$family_id) %>% 
  group_by(family_id) %>%
  filter(n() > 1) %>%
  ungroup()
...
dat_samplec <- dat %>%
  filter(community_id %in% sample_com$community_id)  %>% 
...
p2 <- ggplot(dat, aes(x = factor(income_rel), y = cesd8)) +
  ...
  labs(x = "Perceived Relative Income Position (1–5)", y = "CESD Score (0–24)", 
       title = "CESD Score by Perceived Relative Income Position")
...
pa <- ggplot(dat_samplef, aes(x = factor(family_id), y = cesd8)) +
  geom_boxplot()

pb <- ggplot(dat_samplec, aes(x = factor(community_id), y = cesd8)) + 
  geom_boxplot()
...
```
We observe that the residual variance by far exceeds the variance from any other clusters, indicating that within individual variance dominates, taking up 80\% of the total variance. For the other structures, family is the most important clustering structure in the data, making up around 15\% of the total variance. We observe that around 20\% of the total variance can be attributed to any grouping at all (family, community, or county), giving justification for the use of a hierarchical models. 
