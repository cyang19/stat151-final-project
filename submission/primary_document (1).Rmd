...
Survey wave fixed effects are included to control for common time shocks affecting all respondents, such as nationwide changes in economic conditions, information environments, the effect of COVID overall, and changing mental health resources over time. This ensures that identification comes from variation in local pandemic conditions relative to broader national trends.

Also, we include a small set of individual-level controls—age, self rated health, gender, education, marital status, and urban residence. These controls were chosen because they are well-established correlates of depression. Together, these modeling choices allow us to isolate the association between COVID-19 policy stringency, pandemic severity, and changes in depressive symptoms while accounting for the complex longitudinal and hierarchical structure of the CFPS data.

Our key treatment variables—policy stringency and COVID-19 deaths—are measured as three-month moving averages. We use moving averages for two reasons. First, both policy stringency and reported mortality can exhibit short-term volatility due to reporting noise and temporary policy adjustments; smoothing reduces sensitivity to small, transitory fluctuations that are unlikely to reflect meaningful changes in individual mental health. Second, we expect the psychological effects of prolonged containment measures and sustained pandemic severity to accumulate over time rather than operate instantaneously. A rolling average therefore better reflects sustained exposure to restrictive policies and local pandemic conditions than a single-month measure.
...
Our research has several limitations, primarily in regards to our data. First, numerous reports have suggested China’s official COVID-19 data is unreliable and under-reported. This compromise in data integrity would certainly cast doubt on our model results, as an increased case/death count could change the effect size. Additionally, survey times were not standardized within years, so our data does fully not capture the onset of either waves of lockdowns, and thus we only observe a small snapshot of variance of public health and stringency. For future work, we also hope to explore the effect of COVID-19 on mental health outcomes through an economic lens, such as GDP or unemployment (we were unfortunately missing data of the needed granularity). Furthermore, since our research yielded the unintuitive result that stringency doesn’t explain mental health outcomes within waves, we would hope to explore which features do explain this variance.  
